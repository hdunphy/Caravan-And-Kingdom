...
export const ConstructionSystem = {
    build(state: WorldState, settlementId: string, buildingType: BuildingType, hexId: string, config: GameConfig, silent: boolean = false): boolean {
        ...
        if (!silent) console.log(`[Construction] Built ${buildConfig.name} at ${hexId} for ${settlement.name}`);
        return true;
    },
