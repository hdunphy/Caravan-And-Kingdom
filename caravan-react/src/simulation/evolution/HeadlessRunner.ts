...
export class HeadlessRunner {
    static run(globalConfig: GameConfig, options: HeadlessOptions): { state: WorldState, stats: SimulationStats } {
        ...
        for (let i = 0; i < options.ticks; i++) {
            // Apply Faction-Specific Configs during TICKS?
            // This is hard without changing GameLoop.
            // A trick: We swap the global config passed to systems if we iterate manually?
            // No, GameLoop.tick() runs everything.

            // We'll rely on a follow-up step to update Systems to check `faction.aiConfig`.
            loop.tick();
